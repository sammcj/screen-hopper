...
echo "   isolation gap: $GAP mm   clearance: $CLR mm"
...
"$PY" -u "$LIB/build_board.py" finalize \
        ...
        "$OUT/$NAME.kicad_pcb" 2>/dev/null | grep -iE 'finalized'

echo ">> verify"
"$PY" "$LIB/check_board.py" "$OUT/$NAME.kicad_pcb" "${GAP%,*}" "${GAP#*,}" "$CLR" 2>/dev/null
