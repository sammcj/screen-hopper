...
Usage: check_board.py <board.kicad_pcb> [gap_lo] [gap_hi] [clearance_mm] [axis=x|y]

The isolation gap is a strip on one axis: x (vertical strip, smd/tht/compact) or
y (horizontal strip, the header two-row layout). gap_lo/gap_hi bound it on that axis.
"""
import sys
import pcbnew
...
def main():
    ...
    axis = sys.argv[5] if len(sys.argv) > 5 else "x"
    ...
    # isolation: any copper (track or via) crossing the gap on the chosen axis?
    gap = []
    coord = (lambda p: pcbnew.ToMM(p.x)) if axis == "x" else (lambda p: pcbnew.ToMM(p.y))
    for t in b.GetTracks():
        c0, c1 = coord(t.GetStart()), coord(t.GetEnd())
        if max(c0, c1) > gap_l and min(c0, c1) < gap_r:
