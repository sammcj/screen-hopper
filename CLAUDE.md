...
The JSON can be either a legacy single-profile config OR the multi-profile shape `{ "version": 8, "active_profile": 0, "profiles": [...] }`. `config-tool/config-samm-profiles.json` is the canonical sample. It now holds a single profile (remote below the laptop) with profile switching deliberately disabled - no `0xfff30000` binding. It binds F14 to toggle pointer upsampling (`0xfff30011`), F15 to toggle jiggler (`0xfff30010`) and F16 to switch screen (`0xfff20001`). The firmware still supports up to `NPROFILES` slots; add profiles back to the JSON and re-bind a cycle/activate usage to re-enable switching.
...
- `0xfff30010` **Toggle jiggler** - flip the runtime `jiggle_enabled` flag (does NOT touch the persisted `jiggle_interval`). The onboard LED blinks **3 times for ON, 5 times for OFF** so you can confirm the toggle without looking at the host. Both directions also draw a shape via `start_jiggle_toggle_gesture()`, on the main computer and the forwarder at once: **ON draws a circle** (`JIGGLE_CIRCLE`, 20 chords of a radius-140 circle, ~0.8 s), **OFF draws the cross sweep** (`JIGGLE_SWEEP`, down/up/right/left, ~1 s). Both return the cursor exactly to where it started; the circle's chord deltas sum to (0, 0) by construction.

- `0xfff30011` **Toggle pointer upsampling** (experimental, default ON, 3/5 blinks like the jiggler). When no button is held, the absolute report walks each new 125 Hz mouse sample out linearly over `ABS_SMOOTH_SEGMENT_US` (8 ms) in 1 ms steps so the host sees ~1 kHz updates instead of 8 ms hops. Costs one sample interval of pointer latency. Local host (output 0) only: the forwarder has no report queue and drops frames that arrive while its endpoint is busy, so a 1 kHz abs stream to it would lose clicks and key edges. Drags, crossing bursts and everything relative are untouched; the smoother snaps to `cursor_x/y` on any suppressed-abs frame, and on a press edge `cursor_x/y` is pulled back to the host's interpolated position so the drag dead-reckons from where the pointer really is. Segment length is hard-wired for a 125 Hz mouse; a faster mouse would turn it into a ~8 ms low-pass. Not persisted, no wire change.
