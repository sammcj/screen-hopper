...
The JSON can be either a legacy single-profile config OR the multi-profile shape `{ "version": 8, "active_profile": 0, "profiles": [...] }`. `config-tool/config-samm-profiles.json` is the canonical sample. It now holds a single profile (remote below the laptop) with profile switching deliberately disabled - no `0xfff30000` binding, so F14 is unused. It binds F15 to toggle jiggler (`0xfff30010`) and F16 to switch screen (`0xfff20001`). The firmware still supports up to `NPROFILES` slots; add profiles back to the JSON and re-bind a cycle/activate usage to re-enable switching.
